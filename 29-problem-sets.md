...
**Step 2: show the block still shrinks by a constant factor.** We want $(n+3)/2 \le (3/4) n$, which rearranges to $4(n+3) \le 6n$, that is $n \ge 6$. The odd branch is only reached when $n > 10$, so the condition holds every time it matters, with room to spare. Even inputs shrink by a full half in one call, which is better. So:

> **every two calls reduce `n` to at most $3/4$ of its value.**

**Step 3: count.** Starting from `n`, after $2k$ calls the argument is at most $(3/4)^{k n}$. The recursion stops once the argument drops to 10 or below, which happens once $(3/4)^{k n} \le 10$, that is $k = O(\log n)$. Each call does $O(1)$ work of its own. So the running time is $O(\log n)$.

**Step 4: the matching lower bound, which is the half people skip.** The question said $\Theta$, so an upper bound alone is half an answer. No single call reduces the argument by more than a factor of 2 (halving is the best case; adding 3 makes it worse). So after `k` calls the argument is at least $n / 2^{k}$, and reaching the base case at 10 needs $n / 2^{k} \le 10$, that is $k \ge \lg n - \lg 10$. Hence $\Omega(\log n)$.

**Answer: $\Theta(\lg n)$.**

**What earns full marks.** Three sentences: odd is always followed by even, so two calls take `n` to $(n+3)/2 \le 3n/4$; that is a constant-factor shrink so $O(\lg n)$ calls; no call beats halving so $\Omega(\lg n)$.
...
(By the master theorem, $a = 2$, $b = 2$, $W(n) = n^{\log_2 2} = n$, and $f(n) = n^2$ is polynomially larger, so case 3, with the regularity condition $2 (n/2)^2 = n^2/2 \le c n^2$ holding at $c = 1/2$. Either route is fine; the level sum is faster to write.)
...
The second is worth proving to yourself once, because it looks wrong: $n^{1/\lg n} = 2^{\lg n \cdot (1/\lg n)} = 2^1 = 2$.
...
- $\lg^3 n = (\lg n)^3$.
- $5^{\lg \lg n}$. Apply the swap identity with `n` replaced by $\lg n$: $5^{\lg(\lg n)} = (\lg n)^{\lg 5}$. Since $\lg 5 = 2.3219 \dots$, this is $(\lg n)^{2.32}$.
...
The number of bits to write an integer $N$ in binary is $\lfloor \lg N \rfloor + 1$. So
...
Starting from one billion, the answer is $10^9 \cdot 2^{100}$. That is a specific, enormous, **fixed number**. It does not depend on `n`, or on anything else that grows.

**Answer: $\Theta(1)$.**

**The point.** Asymptotic notation describes how a quantity behaves as some variable goes to infinity. If nothing is going to infinity, every answer is $\Theta(1)$, no matter how large the constant. $10^9 \cdot 2^{100} = \Theta(1)$ is the same joke as $10^{85} = O(1)$ in 22.1, and the reflex the question is testing is *identify the variable before answering*. If a problem gives you a fixed horizon, there is no variable, and the answer is a constant.
...
**Line 10.** $n \lg^2 n$ is above `n` and below every $n^{1+\varepsilon}$, since $\lg^2 n = o(n^{\varepsilon})$.
...
**Line 15.** $(\lg^{*} n)^{\lg n}$. Take logs: $\lg n \cdot \lg \lg^{*} n$. Compare against any polynomial $n^{c}$, whose log is $c \lg n$. Since $\lg \lg^{*} n \to \infty$, however slowly, $\lg n \cdot \lg \lg^{*} n$ eventually exceeds $c \lg n$ for every constant $c$. So this function is **above every polynomial**, despite being built from the slowest-growing function in the list. That inversion is the whole point of the entry: $\lg^{*} n$ is tiny, but $\lg n$ copies of it multiplied together is not.

**Line 16.** $(\lg n)^{\lg n} = 2^{\lg \lg n \cdot \lg n} = n^{\lg \lg n}$ by the swap identity, so the two entries are literally the same function written two ways, hence $\Theta$ of each other. Their common log is $\lg n \cdot \lg \lg n$, which beats line 15's $\lg n \cdot \lg \lg^{*} n$ because $\lg \lg n$ dominates $\lg \lg^{*} n$.

**Line 17.** $n^{\sqrt{n}}$ has log $\sqrt{n} \cdot \lg n$, which beats $\lg n \cdot \lg \lg n$ because $\sqrt{n}$ beats $\lg \lg n$ outright.
...
**Line 19.** $2^{n}$ over $1.001^{n}$: the ratio $(2/1.001)^{n} \to \infty$. Two exponentials with different bases are never $\Theta$ of each other.
...
That is the Fibonacci recurrence, so $T(n) = F(n+1)$ and $T(n) = \Theta(\varphi^{n})$, though the question did not ask.
...
and the number of terms is $\lg^{*} n$ by definition. The first term is $\lg n$; every later term is at most $\lg \lg n$; so the tail is at most $\lg^{*} n \cdot \lg \lg n$, which is $o(\lg n)$ because $\lg^{*} n$ grows slower than any iterated log. The first term therefore swallows the entire rest of the tree.
...
**Running time.** $|C| \le 4$ because each half returns at most 2 candidates, so line 4's loop runs a constant number of times and each iteration is one $O(n)$ scan. Hence
...
**Why discarding is safe**, which is the only part being graded. Suppose $A[i] < B[j]$. How many elements of the union can be less than or equal to $A[i]$? At most `i` from $A$ (its own prefix) and at most $j - 1$ from $B$ (everything before $B[j]$, since $B[j] > A[i]$ and $B$ is sorted). So the rank of $A[i]$ in the union is at most $i + j - 1 \le k - 1 < k$. Every element of `A[1..i]` has rank at most that, so **none of them is the k-th**, and all `i` of them are below it. Discard them and reduce `k` by `i`. The symmetric argument covers the other branch.

**Running time.** Lines 4 and 5 ensure $i + j = k$ whenever both arrays are long enough, so one of them is at least $k/2$ and the discarded prefix has at least $\lfloor k/2 \rfloor$ elements in the common case. Thus `k` at least halves each call:
