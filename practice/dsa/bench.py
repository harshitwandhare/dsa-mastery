...
def measure(fn: Callable, make_input: Callable[[int], tuple], sizes: list[int],
    ...
    for n, t in results:
        if prev_t and prev_t > 0 and prev_n:
            ...
            if t < 1e-3 and time_ratio > 1.3:
                ...
                implies = ("O(1)" if k < 0.3 else
                           "O(log n)" if k < 0.6 else
                           "O(n)" if k < 1.3 else
                           "O(n log n)" if k < 1.6 else
                           "O(n^2)" if k < 2.4 else "worse")
...
def contains_slow(nums, needle):
    return needle in nums            # O(n) scan


def contains_fast(nums_set, needle):
...
def main() -> None:
    ...
    report("Two Sum -- brute force O(n^2)",
           measure(two_sum_bruteforce, make_two_sum, sizes))
    report("Two Sum -- hash map O(n)",
           measure(two_sum_hashmap, make_two_sum, [10_000, 20_000, 40_000, 80_000]))

    def make_list_lookup(n):
        nums = list(range(n))
        return (nums, -1)            # worst case: not present

    def make_set_lookup(n):
        return (set(range(n)), -1)

    report("`x in list` -- O(n)",
           measure(contains_slow, make_list_lookup, [100_000, 200_000, 400_000]))
    report("`x in set` -- O(1)",
           measure(contains_fast, make_set_lookup, [100_000, 200_000, 400_000]))

    report("string += in a loop -- O(n^2)",
           measure(build_string_slow, lambda n: (n,), [20_000, 40_000, 80_000]))
    report("''.join(list) -- O(n)",
