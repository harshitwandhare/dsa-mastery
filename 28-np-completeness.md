...
> **Definition.** A **polynomial-time many-one reduction** from problem A to problem B, written $A \le _p B$, is a polynomial-time computable function $f$ mapping instances of A to instances of B such that
>
> ```
...
$A \le _p B$ means **A is no harder than B**. It says: if I could solve B, I could solve A, by transforming and calling B's solver. So:
...
**Reductions compose.** If $A \le _p B$ and $B \le _p C$, then $A \le _p C$, because the composition of two polynomial functions is polynomial. This is what lets you use any previously-proven NP-complete problem as your starting point rather than going back to SAT every time.
...
| **EUCLIDEAN TSP** | $1 + \varepsilon$ for any eps | a PTAS exists in the plane |
| **SET COVER** | $\Theta(\log n)$ | and $\ln n$ is optimal unless P = NP |
| **INDEPENDENT SET** | none within $n^{1-\varepsilon}$ | essentially inapproximable |
...
**2. Exact exponential algorithms with better constants.** $O(2^{n} \cdot n^2)$ DP over subsets for TSP (Held-Karp) beats $O(n!)$ brute force enormously and is exact. Branch and bound with good pruning solves large real instances.

**3. Fixed-parameter tractability.** Vertex cover is solvable in $O(2^{k} \cdot n)$, which is fast when the cover is small even if the graph is large. The parameter, not the input size, carries the exponent.
