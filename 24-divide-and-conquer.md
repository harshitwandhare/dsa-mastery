...
**Combine lemma.** `MERGE` turns two sorted subarrays into one sorted subarray containing exactly their union, in $\Theta(n_1 + n_2)$ time.
...
**Why that line is correct.** At the moment $R[j]$ is copied, every element still in `L[i..n1]` is greater than $R[j]$ (otherwise it would have been copied first) and every one of them originally sat at an index left of $R[j]$'s. So each forms a crossing inversion with $R[j]$, and there are exactly $n_1 - i + 1$ of them. Conversely every crossing inversion $(i, j)$ is counted exactly once, at the moment $A[j]$ is copied. Therefore the count is exact.
...
**Partition invariant.** At the start of each iteration of line 3, for any index `k`: if $p \le k \le i$ then $A[k] \le x$; if $i+1 \le k \le j-1$ then $A[k] > x$; if $k = r$ then $A[k] = x$. Initialization, maintenance, and termination follow the same drill as `MERGE`, and this is a standard exercise, so be able to write it.
...
The difference from quicksort is that we recurse into **one** side, not both. With a good split, $T(n) = T(n/2) + \Theta(n)$, which by master theorem case 3 is **$\Theta(n)$**, not $n \log n$. The geometric series $n + n/2 + n/4 + \dots$ sums to $2n$.
...
**Why the pivot is good.** Consider the $\lceil n/5 \rceil$ group medians. At least half of them are $\ge x$, that is at least $n/10$ of them. Each such median has 2 elements in its own group that are `>= it`, hence $\ge x$. So at least $3n/10$ elements are $\ge x$. Symmetrically at least $3n/10$ are $\le x$. Therefore **each recursive side has at most $7n/10$ elements.**

(The careful version of the count is $3(\lceil n/5 \rceil/2 - 2) \ge 3n/10 - 6$, where the $-2$ discards the group containing x and the possibly-short last group. The $-6$ is a lower-order term and does not change the answer.)
...
Prove it by substitution to be safe: claim `T(n) <= cn`. Then `T(n) <= c(n/5) + c(7n/10) + an = (9c/10)n + an <= cn` provided $c/10 \ge a$, that is $c \ge 10a$. Works.
...
*Why.* Choosing an ordered sequence of `k` distinct items can be done in $n(n-1) \dots (n-k+1) = n!/(n-k)!$ ways. Each unordered subset is counted once per ordering of its `k` elements, which is $k!$ times. Divide.
...
For one die, $E[X] = (1+2+ \dots +6)/6 = 3.5$. Note that 3.5 is not a possible outcome. Expectation is an average, not a prediction, and "expected running time" means the average over the algorithm's own coin flips, not a guarantee about any single run.
...
*Why.* $E[X]$ is an average of non-negative values. If the event $X \ge c$ had probability $p$, those outcomes alone contribute at least `cp` to the average, so `E[X] >= cp`, so $p \le E[X]/c$.
...
Quicksort's running time is dominated by comparisons, so count those. Let $z_1 < z_2 < \dots < z_n$ be the array elements **in sorted order** (the algorithm does not know this order; we are only using it to name things). Write `Z_ij = {z_i, z_i+1, ..., z_j}` for the block of elements between them inclusive.
...
Element $z$ appears in a subproblem deeper than `m` only if $X_m \ge 1$. Markov, with $c = 1$:
...
- Name the sorted order $z_1 < \dots < z_n$ and define the indicator `X_ij` for "$z_i$ compared to $z_j$".
...
3. Therefore $2^{h} \ge n!$, so $h \ge \lg(n!)$.
...
Applied to searching a sorted array with 3-way comparisons: n possible answers, branching 2, so $h \ge \lg n$, which proves binary search is optimal. Applied in file 28 to much larger effect.
...
Split each into halves: $x = a \cdot 10^{n/2} + b$ and $y = c \cdot 10^{n/2} + d$. Then
...
**Naive.** Try every alignment, compare up to m characters: $\Theta((n - m + 1) \cdot m)$, so `O(nm)`.
...
$T(n) = T(n-1) + T(n-2) + O(1)$, which by the subtract-and-conquer rule is $\Theta(\varphi^{n})$, exponential. And it is exponential for a stupid reason: `FIB(5)` computes `FIB(3)` twice, `FIB(2)` three times, `FIB(1)` five times. The distinct subproblems number only `n`; we are recomputing them $\varphi^{n}$ times.
