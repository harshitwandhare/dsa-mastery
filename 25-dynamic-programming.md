...
Fibonacci is the minimal example. `FIB(n) = FIB(n-1) + FIB(n-2)` has optimal substructure trivially and massive overlap. Naive: $\Theta(\varphi^{n})$. Memoized: $\Theta(n)$, because there are only `n` distinct subproblems and each is computed once.
...
**States** $n \cdot W$, **work per state** $O(1)$, so **`Theta(nW)`**.
...
Read that carefully, because it is the whole trick: **pushing a subtree down one level costs the total frequency of that subtree, once.** The $\sum_i F[i]$ term is the price of the single extra level paid by every key, and the root's own contribution $F[r] \cdot 1$ is included in it.
...
**Setup.** Sort jobs by finish time. Define $p(i)$ = the largest index $j < i$ with $f[j] \le s[i]$, that is the last job that does not conflict with i. Computing all $p(i)$ takes $O(n \log n)$ by binary search.
...
**Subproblem definition.** Sort the jobs so that $f[1] \le f[2] \le \dots \le f[n]$. Let $p(i)$ be the largest index $j < i$ such that $f[j] \le s[i]$, and $p(i) = 0$ if no such job exists. Define `OPT[i]` to be the maximum total weight of a set of mutually compatible jobs chosen from $\{1, \dots , i\}$.
...
*Case B: job i is in S.* Every other job `j` in S has $f[j] \le s[i]$, because S is compatible and jobs are sorted by finish time, so $j \le p(i)$. Hence $S - \{i\}$ is a compatible set drawn from `{1..p(i)}`, giving `weight(S) <= v[i] + OPT[p(i)]`.
...
**Correctness.** By induction on i. `OPT[0] = 0` is correct since no jobs are available. For $i \ge 1$, assume `OPT[j]` is correct for all $j < i$. By the optimal substructure argument above, the maximum weight over `{1..i}` equals `max(OPT[i-1], v[i] + OPT[p(i)])`, and both terms are correct by the hypothesis since $i-1 < i$ and $p(i) < i$. Line 5 computes exactly that. QED
...
**Defining the state too richly.** If your state includes a whole set, you have $2^{n}$ states and no algorithm. Bitmask DP ($O(2^{n} \cdot n)$ for travelling salesman) is sometimes exactly right, but only when n is around 20. Say so explicitly if you use it.
...
8. $n \cdot n \cdot \log n$ states, $O(n)$ work each, so $O(n^3 \log n)$.
