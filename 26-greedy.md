...
*Feasibility.* Since activity 1 has the earliest finish time of all activities, $f[1] \le f[j]$. Every other activity `k` in $O$ is compatible with `j` and finishes later than `j`, so it starts at or after $f[j] \ge f[1]$. Therefore activity 1 does not overlap any of them, and `O'` is a valid set of non-overlapping activities.
...
Let $g_1, g_2, \dots$ be greedy's picks in order and $o_1, o_2, \dots$ be any other feasible solution's picks in order.

*Claim: $f[g_k] \le f[o_k]$ for all k.*

*Base.* $g_1$ has the earliest finish time of all activities, so $f[g_1] \le f[o_1]$.

*Step.* Assume $f[g_{k-1}] \le f[o_{k-1}]$. Since $o_k$ is compatible with $o_{k-1}$, we have $s[o_k] \ge f[o_{k-1}] \ge f[g_{k-1}]$, so $o_k$ was available to greedy at step k. Greedy picks the compatible activity with the smallest finish time, so $f[g_k] \le f[o_k]$.

*Conclusion.* Suppose the other solution has more activities, $m > |G|$. Then $o_{|G|+1}$ exists, and $s[o_{|G|+1}] \ge f[o_{|G|}] \ge f[g_{|G|}]$, so it was compatible with greedy's last pick and greedy would have taken it. Contradiction. Hence $|G| \ge m$. QED
...
*Proof.* Let $T$ be an optimal tree, and let $a$ and $b$ be two siblings at maximum depth (an optimal tree is full, so a deepest node has a sibling). Without loss of generality $f[a] \le f[b]$ and $f[x] \le f[y]$. Since x and y have the two smallest frequencies overall, $f[x] \le f[a]$ and $f[y] \le f[b]$.
...
Both factors are non-negative: $f[a] \ge f[x]$ by choice of x, and `depth(a) >= depth(x)` because a is at maximum depth. So `cost(T') <= cost(T)`, and since T was optimal, `T'` is optimal too. Swap y with b similarly to get `T''`, optimal, with x and y as deepest siblings. QED
...
| Shortest path with negative edges | Dijkstra | $s \to a$ weight 2, $s \to b$ weight 1, $b \to a$ weight -2. Dijkstra finalizes $a$ at 2, true distance is -1. |
...
1. Sort by increasing size. Adjacent exchange: if file `i` of size $a$ precedes file `j` of size $b$ with $a > b$, swapping them decreases the total cost by $a - b > 0$, because the total cost is $\sum_k (n - k + 1) \cdot s[k]$, so earlier positions carry larger multipliers and should hold smaller files. An inversion-free order is exactly increasing size.

2. **Shortest processing time first.** Total completion time is $\sum_k (n - k + 1) \cdot t[k]$, the same expression as problem 1, so the same adjacent-exchange argument applies: any adjacent pair out of increasing-time order can be swapped to strictly reduce the sum.
